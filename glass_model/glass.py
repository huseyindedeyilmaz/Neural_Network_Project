from glass_model.loss import FocalLoss
from collections import OrderedDict
from torchvision import transforms
from torch.utils.tensorboard import SummaryWriter
from glass_model.model import Discriminator, Projection, PatchMaker
...
import torch
import tqdm
import glass_model.common as common
import glass_model.metrics as metrics
import cv2
import glass_model.utils as utils
import glob
import shutil
...
class GLASS(torch.nn.Module):
    ...
    def trainer(self, training_data, val_data, name):
        ...
        if self.distribution == 1:
            ...
            os.makedirs(f'./neural_network/results/judge/avg/{self.svd}', exist_ok=True)
            cv2.imwrite(f'./neural_network/results/judge/avg/{self.svd}/{name}.png', avg_img)
            return self.svd
        ...
        for i_epoch in pbar:
            ...
            if (i_epoch + 1) % self.eval_epochs == 0:
                ...
                eval_path = './neural_network/results/eval/' + name + '/'
                train_path = './neural_network/results/training/' + name + '/'
    ...
    def _evaluate(self, images, scores, segmentations, labels_gt, masks_gt, name, path='training'):
        ...
        for i in range(len(defects)):
            ...
            full_path = './neural_network/results/' + path + '/' + name + '/'
